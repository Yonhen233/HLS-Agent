...
class VivadoHLSAdapter:
    ...
    def create_project(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if firmware_dir.exists() and firmware_dir.is_dir():
            ...
            tcl_path = bridge.create_project_tcl(
                ...
                array_partition_maximum_size=arguments.get("array_partition_maximum_size"),
            )
            return {
                "status": "success",
    ...
    def run_csim(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        stage_tcl = work_dir / "csim_stage.tcl"
        if not stage_tcl.exists():
            return error_result(
                build_error(
                    "VivadoSynthesisError",
                    "Real csim requires the csim_stage.tcl generated by vivado.create_project.",
                    recoverable=True,
                    source="vivado.run_csim",
                    suggested_action="Create the Vivado HLS project before running C simulation.",
                    details={"work_dir": str(work_dir), "expected_tcl": str(stage_tcl)},
                )
            )
        ...
        if code_file is None:
            return error_result(
                build_error(
                    "VivadoSynthesisError",
                    "No design cpp file found for real C simulation.",
                    recoverable=True,
                    source="vivado.run_csim",
        ...
        if self.hls_toolchain == "vitis_hls":
            return error_result(
                build_error(
                    "VivadoSynthesisError",
                    "Standalone real CSim is currently implemented for vivado_hls only.",
                    recoverable=True,
                    source="vivado.run_csim",
        ...
        return {
            ...
            "project_dir": result.get("project_dir") or str(work_dir),
            "toolchain": self.hls_toolchain,
